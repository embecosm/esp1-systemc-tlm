...
//! No delay added here - the UART will have modeled that
...
//! Wait to be notified via the SystemC event TermSyncSC::ioEvent that there is a
//! character available.
...
//! The parent is initalized using TermSyncSC::xtermSetup()

//! @return   0 on success, -1 on failure, with an error code in errno

int
TermSyncSC::xtermInit()
{
...
void
TermSyncSC::xtermKill( const char *mess )
{
...
void
TermSyncSC::xtermLaunch( char *slaveName )
{
  char *arg;
...
//! then notify a SystemC event (TermSyncSC::ioEvent).
...
Fd2Inst *TermSyncSC::instList = NULL;


//! Static handler for SIGIO

//! Must be static, since sigaction cannot cope with a member function. The
//! static member list TermSyncSC::instList holds details of each instance of this
...
void
TermSyncSC::ioHandler( int        signum,
		   siginfo_t *si,
		   void      *p )
{
  ...
  case -1:
    perror( "TermSyncSC: Error on handler select" );
    return;
...
unsigned char
TermSyncSC::xtermRead()
{
  // Have we an xterm?

  if( ptSlave < 0 ) {
    std::cerr << "TermSyncSC: No channel available for read" << std::endl;
    return -1;
  }
  ...
  if( read( ptSlave, &ch, 1 ) != 1 ) {
    perror( "TermSyncSC: Error on read" );
    return  0;
  }
  else {
    return  ch;
...
void
TermSyncSC::xtermWrite( unsigned char  ch )
{
  // Have we an xterm?

  if( ptSlave < 0 ) {
    std::cerr << "TermSyncSC: No channel available for write" << std::endl;
    return;
  }
  ...
  if( write( ptSlave, &ch, 1 ) != 1 ) {
    std::cerr << "TermSyncSC: Error on write" << std::endl;
  }
}	// xtermWrite()
